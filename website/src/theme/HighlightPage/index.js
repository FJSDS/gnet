...
function prTags(numbers) {
  return numbers.map(number => ({
    ...
    permalink: `https://github.com/FJSDS/gnet/commit/${number}`,
    style: 'secondary'
  }));
}
...
function GetText({release}) {
  if (release == 'features') {
    ...
    return <p>This change was made available in <Link to={`https://github.com/FJSDS/gnet/releases/tag/v${release}/`}>{release}</Link>.</p>;
  }
}
